...
export class PermissionService {
  public check(
    ...
    switch (capability) {
      case "assets.read.shared":
        return this.canReadSharedAsset(
          context
        );

      case "assets.read.personal":
        return this.canReadPersonalAsset(
          context
        );

      case "assets.upload.personal":
        return this.canUploadPersonalAsset(
          context
        );

      case "assets.upload.shared":
        return this.canUploadSharedAsset(
          context
        );

      case "assets.manage.own":
        return this.canManageOwnAsset(
          context
        );

      case "assets.attach.campaign":
        return this.canAttachAssetToCampaign(
          context
        );

      case "assets.moderate":
        return this.canModerateAssets(
          context
        );

      case "assets.delete.shared":
        return this.canDeleteSharedAsset(
          context
        );

      case "campaign.read":
        return this.canReadCampaign(
          context
        );

      case "campaign.write":
        return this.canWriteCampaign(
          context
        );

      case "users.manage.trust":
        return this.canManageTrust(
          context
        );
  ...
  /**
   * Determines the processing status assigned by the
   * server when a user uploads a campaign asset.
   */
